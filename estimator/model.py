...
import estimator.net as net
from estimator.featurize import TreeFeaturizer
...
class BaoRegression:
    def __init__(self, verbose=False, have_cache_data=False, alpha=0.1):
        self.__net = None
    ...
    def quantile_loss(self, preds, targets):
        diff = preds - targets
        mask = (diff.ge(0).float() - self.alpha).detach()
        loss = mask*diff
        return loss.mean()

    def quantile(self, preds, targets):
        num_below = 0
        for i in range(len(preds)):
            num_below += 1 if preds[i] >= targets[i] else 0
        return num_below / len(preds)
    ...
    def fit(self, X, y):
        ...
        valid_pairs = list(zip(X_valid, y_valid))
        valid_dataset = DataLoader(valid_pairs,
                             batch_size=16,
                             shuffle=True,
                             collate_fn=collate)
        ...
        # for epoch in range(50):
            ...
            for x, y in dataset:
                ...
                loss.backward()
                optimizer.step()
            train_loss = loss_accum / len(dataset)
            ...
            qtl = self.quantile(all_preds, all_targets)
            if epoch % 1 == 0:
                self.__log("Epoch", epoch, "training loss:", train_loss, "coverage rate:", qtl)
            
            if abs(qtl - self.alpha) < 0.2:
                best_model = copy.deepcopy(self.__net)
                final_qtl = qtl

            if len(losses) > 10 and losses[-1] < 0.1:
                last_two = np.min(losses[-2:])
                if last_two > losses[-10] or (losses[-10] - last_two < 0.00001):
                    self.__log("Stopped training from convergence condition at epoch", epoch)
                    break
        else:
            self.__log("Stopped training after max epochs")
        if best_model == None:
            print("no model satisfies the coverage rate constraint")
